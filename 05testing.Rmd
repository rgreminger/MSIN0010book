...
**`t.test(mydata1, mydata2, alternative, mu, var.equal, conf.level)`**<br>
...
`var.equal`: do you want to assume the population variances are equal? (options: TRUE, FALSE)<br>
`conf.level`: confidence level of the test ($1-\alpha$)<br>
...
t.test(SMI, CAC, alternative="two.sided", mu=0, var.equal=FALSE, conf.level=0.95)
```
